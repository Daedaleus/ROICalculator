...
impl eframe::App for RoiCalculator {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.add(egui::Slider::new(&mut self.application_count, 1..=20).text("Application count"));

            ui.separator();

            match self.inputs.len().cmp(&self.application_count) {
                std::cmp::Ordering::Less => self.inputs.resize(self.application_count, Input::default()),
            ...
            for (index, input) in self.inputs.iter_mut().enumerate() {
                ui.horizontal(|ui| {
                    ...
                    ui.text_edit_singleline(&mut input.start).labelled_by(label_start.id);
                });
                ui.horizontal(|ui| {
                    let label_end = ui.label(format!("{} End: ", index + 1));
                    ui.text_edit_singleline(&mut input.end).labelled_by(label_end.id);
                });
            }
